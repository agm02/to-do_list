...
export default function Body() {
    ...
    function checkTask(taskId: number) {
        ...
        setTasks(prevTasks => prevTasks.map(task => task.id === taskId ? { ...task, conclusionDate: new Date(), checked: !task.checked } : task))
    ...
    return (
        <main className="body">
            ...
            <div className="body-header-container">
                <div className="body-input-container">
                    <div className="display-flex-column">
                        ...
                        <input className="form-control" type="text" id="tarefa" name="tarefa" value={inputTask} onChange={(e) => setInputTask(e.target.value)} style={{ padding: "10px 16px", width: "400px" }}></input>
                    </div>
                    <Button title="Adicionar Tarefa" color={Colors.PRIMARY} padding="10px 16px" disabled={inputTask === ''} onClick={() => addTask()} />
                </div>

                <Pagination currPage={currPage} maxPage={maxPage} itemsPerPage={itemsPerPage} allItemsPerPage={allItemsPerPage}
            ...
            <Table tasks={renderTasks()} checkTask={(_, taskId) => checkTask(taskId)} handleRemove={(_, taskId) => removeTask(taskId)} />
